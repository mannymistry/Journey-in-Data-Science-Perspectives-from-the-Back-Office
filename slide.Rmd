...
--
  ...
  + Version control (Happy with GitR)
  
<!-- **Screenshot and link to Happy with GitR here**   -->
--

  + Incredibly kind and supportive community (R)

<!-- **Screen Shot from Stackoverflow goes here** -->
...
--
  ...
  + RStudio IDE made it that much easier
...
--
  ...
  + Heavy reliance on the R community and Stack Overflow 
...
1) 
If it is a work thing and you can't get access to R do it at home, Create a mock data set with similar properties to the data at work

2)
This was critical so that I did not lose motivation
It also helped me see the big picture as to what I wanted to accomplish
...
### When I got stuck 
  ...
  + Read the function documentation - skip straight to the example and attempt to replicate 
  + Stackoverflow - Has someone else run into the same issue? Don't forget to use tags
  + Stackoverflow - Ask a reproducible question
...
<!-- A screenshot of an RMarkdown document one with alot of text -->
...
### When to consider using R: A back-office example
  ...
  + When a new counterparty is brought on board multiple sheets need to be updated manually in order to account for this
  + One a typo in the counterparty name could throw off a variety of lookups you have in place
  + You did not create the macro use in sorting and that person has also left the company
...
*Most if not all remixed/adapted from Greg Wilson Teaching Tech Together - "The Art of the Cold Call"
